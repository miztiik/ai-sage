...
var __get_suffix = length(deploymentParams.enterprise_name_suffix) < 3
  ? deploymentParams.enterprise_name_suffix
  : substring(deploymentParams.enterprise_name_suffix, 0, 3)

var __kv_name = '${key_vault_params.name_prefix}-${deploymentParams.loc_short_code}-${__get_suffix}-${__uniq_str_1}-${deploymentParams.global_uniqueness}'
...
output kv_name string = r_kv.name

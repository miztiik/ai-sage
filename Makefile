...
func: ## Trigger Only Funtion code deployments
	bash deployment_scripts/deploy_func_v2.sh

docker: ## Build docker image
	bash deployment_scripts/build_and_push_img.sh

powerup: ## Add permissions to deployment id (NOT WORKING - WIP)
	bash deployment_scripts/add_perms_to_deployement_id.sh

spice: ## Deploy k8s_utils
	bash app/k8s_utils/bootstrap_cluster/setup_kubeconfig.sh
	bash app/k8s_utils/bootstrap_cluster/deploy_dashboard.sh

destroy: ## Delete deployments without confirmation
	@echo "⚡⚠️ I hope you are sure to delete everything?⚡"
	bash deployment_scripts/destroy.sh shiva
